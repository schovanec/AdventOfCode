...
namespace day23
{
    public sealed class IntcodeMachine
    {
...
#if false
        public static bool RunProgram(long[] program, Action<long> write, params string[] inputs)
        {
            var inputQueue = new Queue<long>();
            foreach (var line in inputs ?? Enumerable.Empty<string>())
            {
                foreach (var ch in line)
                    inputQueue.Enqueue(ch);

                inputQueue.Enqueue(10); // newline
            }

            return RunProgram(program, write, inputQueue);
        }
#endif

        public bool IsHalted => CurrentInstruction == Instruction.Halt;

        public ExecuteResult StepMany(Func<long?> input)
        {
            while (true)
            {
                var result = Step(input ?? (() => default));
        ...
        public ExecuteResult StepMany(long input)
        {
            ...
            return StepMany(() => inputQueue.TryDequeue(out var result) ? result : default);
        }
#endif

        public ExecuteResult Step(Func<long?> input)
        {
        ...
        private ExecuteResult ReadInput(Func<long?> input)
        {
            var inputValue = input?.Invoke();
